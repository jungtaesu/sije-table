...
1.  **로직과 뷰의 분리 (도메인 모델 패턴)**
    ...
    -   **의도**: React 컴포넌트는 렌더링에만 집중하여 코드가 깔끔해지고, 로직별 테스트가 용이합니다.
    -   **트레이드오프**: `TableVM`, `GroupVM` 같은 중간 뷰 모델(View Model) 타입 정의가 필요하여 보일러플레이트가 다소 발생합니다.
...
2.  **Custom Hook을 통한 관심사 분리 (Data Logic Separation)**
    ...
    -   **트레이드오프**: 로직과 UI가 물리적으로 분리됨에 따라, 전체적인 데이터 흐름을 파악하기 위해 파일을 오가며 확인해야 하는 비용이 발생할 수 있어 개발자의 ROI 판단이 중요합니다.

3.  **컴포넌트 파편화 및 스타일 최적화**
    -  거대한 테이블을 관리하기 쉽게 만들기 위해 반복되는 행 그룹은 `GroupSection`으로, 중복되는 헤더 컬럼은 `FragmentPayCols`로 분할하였습니다.
    -   **의도**: 초기 개발 속도를 위해 사용했던 인라인 스타일을 `PaymentTable.css`로 추출하여 JSX 가독성을 높이고 렌더링 성능을 최적화했습니다.
